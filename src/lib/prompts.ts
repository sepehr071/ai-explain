...
const THINKER_BALANCED = `You are an expert researcher and infographic content planner. Your job is to take any question and produce a structured content plan that a visual designer will use to create an infographic.
...
const THINKER_DETAILED = `You are an expert researcher and infographic content planner. Your job is to take any question and produce a structured content plan that a visual designer will use to create an infographic.

## YOUR TASK
Given a question, produce a structured content plan. Focus on:
- Factual accuracy and depth
- Clear organization into 5-7 distinct sections
- Identifying what visual diagrams best explain each concept
- Providing concrete data points, numbers, and specifics
...
[Add up to 7 sections total if the topic warrants it. Go deep.]
...
2. **[Diagram type]:** [detailed description with labels, connections, values]
3. **[Diagram type]:** [detailed description with labels, connections, values]
[Minimum 5 diagrams, aim for 6-8]
...
- If the topic involves a process, use a flowchart. If it involves comparison, use a versus layout. If it involves change over time, use a timeline. Match the visual to the content.
- Default to including 1-2 image prompts. Only skip images for pure algorithm/math/code topics.
- Provide deep analysis with nuance — include edge cases, exceptions, historical context, and counterpoints where relevant.
- For each section, provide 3-6 detailed bullet points rather than 2-4.`;

export function buildThinkerPrompt(detailLevel: DetailLevel = "balanced"): string {
  if (detailLevel === "detailed") {
    return THINKER_DETAILED;
  }
  // "balanced" and "short" (safety fallback) both return the balanced prompt
  return THINKER_BALANCED;
}
...
export function buildShortCoderPrompt(preset: StylePreset): string {
